...
void readBicorpus(std::istream &is, SentenceList &huSentenceList,
  ...
  while ((is.good()) && (!is.eof())) {
    ...
    split(line, halfs);
    if (halfs.size() != 2) {
      std::cerr << "Incorrect bicorpus file: " << halfs.size()
                << " records in line " << huSentenceList.size() << std::endl;
      throw "data error";
    }
...
void writeBicorpus(std::ostream &os, const SentenceList &huSentenceList,
...
void DictionaryItems::read(std::istream &is) {
  ...
  while (!is.eof()) {
    ...
    while (true) {
      ...
      // We allow vonyo7's "@" delimiter, and vonyokornai's "@V", "@N" etc.
      // delimiters.
      if ((w.size() <= 2) && (w[0] == '@')) {
        engPart = false;
        delimiter = w;
...
void Dictionary::read(const char *dictionaryFile) { throw "unimplemented"; }

void Dictionary::build(const DictionaryItems &dictionaryItems) {
  throw "unimplemented";
}

void Dictionary::reverse(const Dictionary &dic) { throw "unimplemented"; }

bool Dictionary::lookupWord(const Word &word, DictionaryItems &results) const {
  return false;
}

bool Dictionary::lookupWordSet(const WordList &words,
                               DictionaryItems &results) const {
  return false;
}

void FrequencyMap::add(const Word &word) { ++operator[](word); }

void FrequencyMap::remove(const Word &word) { --operator[](word); }
...
void FrequencyMap::reverseMap(
    FrequencyMap::ReFrequencyMap &reFrequencyMap) const {
  reFrequencyMap.clear();

  for (const_iterator it = begin(); it != end(); ++it) {
    reFrequencyMap.insert(
        FrequencyMap::ReFrequencyMap::value_type(it->second, it->first));
  }
}
...
void removeEnglishStopwords(SentenceList &enSentenceList) {
  // Mar megbocsasson mindenki, hogy ezt programkodban rogzitem, de rogzitem.
  const char *enStopwordsC[] = {"the", "it",
...
void removeStopwords(SentenceList &huSentenceList,
                     SentenceList &enSentenceList) {
...
void TransLex::build(const DictionaryItems &dictionaryItems) {
  ...
  for (int i = 0; i < dictionaryItems.size(); ++i) {
    if ((dictionaryItems[i].first.size() == 1) &&
        (dictionaryItems[i].second.size() == 1)) {
  ...
  std::cerr << added << " items added to TransLex, " << ignored
            << " multiword items ignored." << std::endl;
}
...
bool TransLex::isPresent(const Word &huWord, const Word &enWord) const {
  ...
  for (WordMultimapIt it = dictInterval.first; it != dictInterval.second;
       ++it) {
    if (it->second == enWord) {
      return true;
    }
...
void IBMModelOne::build(const SentenceList &huSentenceList,
                        const SentenceList &enSentenceList) {
  transProbs.clear();
  ...
  for (TransProbs::iterator it = transProbs.begin(); it != transProbs.end();
       ++it) {
    it->second /= huProb[it->first.first];
  }
}

void IBMModelOne::reestimate(const SentenceList &huSentenceList,
                             const SentenceList &enSentenceList) {
  throw "unimplemented";
}
